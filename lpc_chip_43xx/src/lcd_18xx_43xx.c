...
static uint32_t getClockDivisor(uint32_t desired_clock)
{
  ...
  clk = Chip_Clock_GetBaseClocktHz(CLK_BASE_LCD);
...
void Chip_LCD_Init(LPC_LCD_T *pLCD, LCD_CONFIG_T *LCD_ConfigStruct)
{
	...
	/* panel clock divisor */
	if (LCD_ConfigStruct->Clock == 0) {
		/* Setup the LCD clock to use the external clock */
		Chip_Clock_SetBaseClock(CLK_BASE_LCD, CLKIN_CLKIN, true, false);

		/* Use external clock in and bypass the pixel clock divider */
		regValue |= CLCDC_LCDPOL_CLKSEL | CLCDC_LCDPOL_BCD;
	} else {
		/* Setup the LCD clock to use the internal clock and dividers */
		Chip_Clock_SetBaseClock(CLK_BASE_LCD, CLKIN_MAINPLL, true, false);

		regValue |= getClockDivisor(LCD_ConfigStruct->Clock);
	}
	pLCD->POL = regValue;
